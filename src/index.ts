...
type Options = StageSynthesisOptions & {
  ...
  /**
   * Ignore BuildSpecs generated by CodePipeline Construct
   */
  ignoreBuildSpec?: boolean;
...
const convertStack = (stack: Stack, options: Options = {}) => {
  ...
  if (ignoreBuildSpec && template.Resources) {
    if (template.Parameters) {
      template.Parameters = expect.any(Object);
    }
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    Object.values(template.Resources).forEach((resource: any) => {
      if (resource?.Properties?.Source?.BuildSpec) {
        resource.Properties.Source.BuildSpec = expect.any(Object);
      }
    });
  }
